...
mwc) ;&
niri)
	sed -i -E --follow-symlinks "s/(${wayland_vars[@]} exec ${dbus} dwl|${wayland_vars[@]} exec ${dbus} river|${wayland_vars[@]} exec ${dbus} mango|${wayland_vars[@]} exec ${dbus} mwc|${wayland_vars[@]} exec ${dbus} niri|${wayland_vars[@]} exec ${dbus} sway|${wayland_vars[@]} exec ${dbus} qtile start -b wayland|exec ${dbus} startx ~\/.xinitrc-qtile|exec ${dbus} startx ~\/.xinitrc-i3|${wayland_vars[@]} exec ${dbus} Hyprland)/${wayland_vars[@]} exec ${dbus} $op/g" ~/.bash_profile
	exit_compositor
	;;
swayfx)
...
hyprland)
	...
	;;
mangowc)
	sed -i -E --follow-symlinks "s/(${wayland_vars[@]} exec ${dbus} dwl|${wayland_vars[@]} exec ${dbus} river|${wayland_vars[@]} exec ${dbus} mango|${wayland_vars[@]} exec ${dbus} mwc|${wayland_vars[@]} exec ${dbus} niri|${wayland_vars[@]} exec ${dbus} sway|${wayland_vars[@]} exec ${dbus} qtile start -b wayland|exec ${dbus} startx ~\/.xinitrc-qtile|exec ${dbus} startx ~\/.xinitrc-i3|${wayland_vars[@]} exec ${dbus} Hyprland)/${wayland_vars[@]} exec ${dbus} mango/g" ~/.bash_profile
	exit_compositor
	;;
esac
